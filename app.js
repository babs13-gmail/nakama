...
async function loadRealNews() {
  try {
    ...
    // La bannière hero reprend les 3 premières vraies actus, avec un vrai lien
    heroSlides.length = 0;
    liveNews.slice(0, 3).forEach((n) => {
      heroSlides.push({
        tag: "À LA UNE",
        title: n.title,
        desc: n.excerpt,
...
function newsCardHTML(n) {
  const thumbStyle = n.thumb ? ` style="background-image:url('${n.thumb}')"` : "";
  return `
    <a class="news-card" href="${n.link || "#"}" target="_blank" rel="noopener">
      <div class="news-thumb news-thumb-${n.tag}"${thumbStyle}>
